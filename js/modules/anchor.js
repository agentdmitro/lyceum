...
var anchorTop = $anchor.offset().scrollTop;
...
$anchor.on('click', 'a', function (e) {
  ...
  if (!$anchor.hasClass('animate') && !$(sectionId).length == 0) {
    ...
    $('html')
      .animate({ scrollTop: sectionTop - fixedTop }, 100)
      .delay(600)
      .queue(function (next) {
...
const anchorScroll = function (link) {
  if (window.innerWidth < 1024) {
    ...
    $('.anchor__list').animate(
      { scrollLeft: link.offset().left + $('.anchor__list').scrollLeft() - $deff },
      200,
    );
  }
};
